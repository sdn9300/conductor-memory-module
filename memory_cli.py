#!/usr/bin/env python3
"""
CONDUCTOR [8] - Memory Module CLI Harness (Phase 5 / FastMCP Alignment)
...
def cmd_record(args, store: MemoryStore) -> None:
    ...
    try:
        ...
        print(
            f"Invalid event type '{args.event_type}'. Valid types: {[e.value for e in EventType]}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    event = MemoryEvent(
        ...
        job_id=args.job_id,
        candidate_id=args.candidate_id or "sdn9300",
        domain=args.domain,
    ...
    print(
        f"Successfully recorded event {event.event_id} ({event.event_type.value}) for application: {event.application_id or 'N/A'}"
    )
    ...
    if event.application_id:
        ...
        if app:
            print(
                f"Current Status: {app.status.value.upper()} | Company: {app.company} | Role: {app.role_title}"
            )
...
def cmd_status(args, store: MemoryStore) -> None:
    ...
    print(f"  Domain:           {app.domain or 'N/A'}")
...
def cmd_list(args, store: MemoryStore) -> None:
    ...
    if args.status:
        try:
            ...
            print(
                f"Invalid status '{args.status}'. Valid statuses: {[s.value for s in ApplicationStatus]}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    print(
        f"{'APPLICATION ID':<20} | {'STATUS':<20} | {'COMPANY':<20} | {'ROLE':<25} | {'LAST UPDATED'}"
    )
    print("-" * 105)
    for app in apps:
        print(
            f"{app.application_id:<20} | {app.status.value:<20} | {app.company:<20} | {app.role_title:<25} | {app.last_updated.strftime('%Y-%m-%d %H:%M')}"
        )
...
def cmd_stale(args, store: MemoryStore) -> None:
    ...
    print(
        f"{'APPLICATION ID':<20} | {'STATUS':<20} | {'COMPANY':<20} | {'ROLE':<25} | {'LAST ACTIVITY'}"
    )
    print("-" * 105)
...
def cmd_history(args, store: MemoryStore) -> None:
    ...
    if not history:
        print(
            f"No history found for application ID '{args.application_id}'.",
            file=sys.stderr,
        )
        return
    ...
    print("=" * 90)
    print(
        f"EVENT HISTORY FOR APPLICATION: {args.application_id} ({len(history)} events)"
    )
    print("=" * 90)

    for i, event in enumerate(history, 1):
        print(
            f"[{i}] {event.occurred_at.strftime('%Y-%m-%d %H:%M:%S UTC')} | Type: {event.event_type.value} | Source: {event.source_component}"
        )
...
def cmd_cooldown(args, store: MemoryStore) -> None:
    res = store.check_domain_cooldown(args.domain)
    print("=" * 60)
    print(f"DOMAIN COOLDOWN STATUS: {args.domain}")
    print("=" * 60)
    if res["is_blocked"]:
        expires = res["cooldown_expires_at"].isoformat() if res["cooldown_expires_at"] else "N/A"
        print(f"  Status:             BLOCKED (30-day rejection cooldown active)")
        print(f"  Cooldown Expires:   {expires}")
    else:
        print(f"  Status:             CLEAR (No active cooldown)")
    print("=" * 60)


def cmd_rebuild(args, store: MemoryStore) -> None:
    print("Rebuilding derived application records, status transitions, and domain cooldowns from raw events...")
    store.rebuild_derived_state()
    total_apps = len(store.list_applications())
    print(
        f"Rebuild complete! Derived state reconstituted for {total_apps} application(s)."
    )
...
def main():
    ...
    p_record.add_argument(
        ...
        dest="event_type",
        required=True,
        help="Event type (e.g. job_discovered, outreach_sent, etc.)",
    )
    p_record.add_argument(
        "-s", "--source", dest="source", default="cli_manual", help="Source component name"
    )
    p_record.add_argument("-j", "--job-id", dest="job_id", help="Job ID")
    p_record.add_argument("-c", "--company", dest="company", help="Company name")
    p_record.add_argument("-d", "--domain", dest="domain", help="Company/ATS domain")
    p_record.add_argument(
        "--candidate-id", dest="candidate_id", default="sdn9300", help="Candidate ID"
    )
    p_record.add_argument("-r", "--role", dest="role", help="Role title")
    p_record.add_argument(
        "--intent", dest="intent", help="Intent label (for response_classified)"
    )
    p_record.add_argument(
        "--status-override", dest="status_override", help="Status override (for manual_note)"
    )
    p_record.add_argument("-p", "--payload", dest="payload", help="JSON payload string")
    p_record.add_argument(
        "--raw-ref", dest="raw_ref", help="Raw source reference identifier"
    )
    ...
    p_list.add_argument("--candidate-id", help="Filter by Candidate ID (optional)")

    # 4. stale
    p_stale = subparsers.add_parser(
        "stale", help="Find stale applications awaiting response"
    )
    p_stale.add_argument(
        "--days", type=int, default=7, help="Number of days silent (default: 7)"
    )
    ...
    # 6. cooldown
    p_cooldown = subparsers.add_parser("cooldown", help="Check 30-day domain cooldown status")
    p_cooldown.add_argument("--domain", required=True, help="Company/ATS domain to check")

    # 7. rebuild
    p_rebuild = subparsers.add_parser(
        "rebuild", help="Rebuild derived state by replaying all events"
    )
